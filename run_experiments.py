...
def cmd_check_only(tests):
    ...
    for name, cnf_path, expected in tests:
        ...
        print("%-22s  %s" % (name, "VALID" if valid else "INVALID"))
        if valid: passed += 1
        else: failed += 1
    print("-" * 70)
...
def run_one(name, cnf_path, expected):
    ...
    return {
        "name": name, "vars": nvars, "clauses": len(clauses),
        "result": result, "time": t1 - t0, "steps": proof_steps,
        "checker": checker_result, "ok": result == expected,
    }


def print_table(rows):
    print("\n" + "=" * 80)
    print("%-22s %5s %7s %6s %10s %8s %7s" % ("Instance", "Vars", "Cls", "Res", "Time(s)", "Steps", "Status"))
    print("-" * 80)
    for r in rows:
        status = "[OK]" if r["ok"] else "[FAIL]"
        print("%-22s %5s %7s %6s %10.4f %8s %7s" % (
            r["name"][:22], r.get("vars","?"), r.get("clauses","?"),
            r["result"], r.get("time",0), r.get("steps","---"), status
        ))
    print("=" * 80)
